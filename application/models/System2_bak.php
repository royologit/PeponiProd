...
class System2 extends CI_Model
{
    ...
    public function generateInvoice($orderId, $rest = false)
    {
        ...
        if ($rest) {
            ...
            $remainingPrice = intval($orderData->order_price)/$quantity;
            if (isset($tripPeriod->period_json)) {
                $periodData = json_decode($tripPeriod->period_json);
                $remainingPrice -= intval($tripPeriod->down_payment);
                ...
                foreach ($periodData as $index => $period) {
                    if ($remainingPrice > 0) {
                        ...
                        error_log("due date $period->label $period->duedate");
                        ...
                        error_log("Current due date $currentDueDate");
            ...
            if (isset($tripPeriod->down_payment)) {
                ...
                $filePdf = $lib->generate_pdf((object) $invoiceData);
                $this->send_email($orderId, $invoiceId, "reservation", $filePdf, "Invoice $invoiceId - " . $invoiceData->title . ".pdf");
    ...
    public function generateReceipt($invoiceData)
    {
        ...
        $this->send_email($invoiceData->order_id, $invoiceData->id, "invoice receipt", $filePdf, "#".$invoiceData->id." - Receipt");
    }

    public function send_email($orderId, $invoiceId=null, $emailType="reservation", $filePdf=null, $fileName=null, $alert = "")
    {
        ...
        $isPrivate = ($orderData->product_name == null || $order->order_price == null) ? 1 :0;
        $emailViewData = [
            'orderData'         => $orderData,
        ...
        if (count($checkEmailHistory) <= 0) {
            if ($emailType == "reservation") {
                ...
                $customerEmailTitle = $title;
                $adminEmailTitle = "Second Reminder H+7 Forward - Client " . $orderData->order_name . " - " . $title;
                $customerEmailView   = $this->ci->load->view('email/v2/invoice_detail', $emailViewData, true);
        ...
        $this->email->to("finajnjxdwtddba@arxxwalls.com");
        $this->email->from("no-reply@peponitravel.com", "Peponitravel");
        $this->email->subject($adminEmailTitle);
        $this->email->message($customerEmailView);
        if ($filePdf != null) {
    ...
    {
        $this->db->select("Pr.product_id, Pr.product_name, Pr.product_price, Pr.product_duration, Pr.product_airlines, Pkg.package_name, count(Or.order_id) participant");
    ...
    {
        $this->db->select("O.order_id, O.order_name, O.order_phone, O.order_line_id, O.order_email, O.order_start_date, O.order_end_date, O.order_price, O.order_name, V.voucher_code, Sum(Case When In.status = 1
