...
class SystemController extends CI_Controller
{
    ...
    public function Order_Management()
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($orderDetails as $orderDetail) {
                $orderParticipant[] = $orderDetail->age_group_name . ' : ' . $orderDetail->order_detail_quantity . ' X ' . $this->currency($orderDetail->order_detail_price);
            }
                ...
                if (is_array($params["period"]["period_json"]["label"])) {
                    foreach ($params["period"]["period_json"]["label"] as $index => $value) {
                        $period_data[] = array("label"   => $value,
                                               "duedate" => $params["period"]["period_json"]["duedate"][$index]?$params["period"]["period_json"]["duedate"][$index]:$params["period"]["period_json"]["days"][$index],
                                               "price"   => $params["period"]["period_json"]["price"][$index]);
                    }
                    $params["period"]["period_json"] = json_encode($period_data);
                }
                $params["period"]["product_id"] = 0;
                $params["period"]["private_id"] = $params["private_trip"]["id"];
    ...
    public function Create_Private_Trip()
    {
        ...
        unset($params['price']);
        $params['price'] = 0; //price private trip unused value
        ...
        if ($res > 0) {
            ...
            $params["period"]["product_id"] = 0;
            $params["period"]["private_id"] = $res;
            $params["period"]["is_private_trip"] = 1;
            $params["period"]["created_date"] = date("Y-m-d H:m:s");
            $periodTrip = $this->system2->updateTripPeriod($params["period"]);
            $data = array(  "title"     => "Add Private Trip Success",
                            "message"   => "Insert private trip with id " .$res,
                            "status"    => "1",
                    );
            if($periodTrip > 0){
                $data['title'] = "Add Private Trip and Period Payment Success";
            }
        } else {
    ...
    public function Update_Private_Trip()
    {
        ...
        if ($res > 0) {
            ...
            $params["period"]["product_id"] = 0;
            $params["period"]["private_id"] = $private_trip_id;
            $params["period"]["is_private_trip"] = 1;
            $params["period"]["created_date"] = date("Y-m-d H:m:s");
            if($period_id){
                $periodTrip = $this->system2->updateTripPeriod($params["period"], $period_id);
            } else {
                $periodTrip = $this->system2->updateTripPeriod($params["period"]);
            }
            ...
            if($periodTrip > 0){
                $data['title'] = "Update Private Trip and Period Payment Success";
            }
        } else {
    ...
    public function Period_Management()
    {
        ...
        // echo "<pre>" . var_export($this->system2->getInvoices(), true) . "</pre>";
        $content["result"]     = array_merge($this->system2->getTripPeriod(), $this->system2->getPrivateTripPeriod());

        $content["name"]       = "Period";
        $content["product_list"] = $this->system2->getProduct();
        $content["private_list"] = $this->system2->getPrivateTrip();
        $this->load->view("admin/layouts/v2/header", $data);
    ...
    public function Create_Period()
    {
        ...
        if (is_array($params["label"])) {
            foreach ($params["label"] as $index => $value) {
                if($params["interval"][$index]==1){
                    $period_data[] = array("label"   => $value,
                    "days" => $params["days"][$index],
                    "price"   => $params["price"][$index]);
                }
                else{
                    $period_data[] = array("label"   => $value,
    ...
    public function Add_Payment()
    {
       ...
       $this->system2->Add_Payment($this->input->post("invoice_id"));
    }
    ...
    {
        $res = $this->system2->delete_Participant($this->input->post("order_id"));
        return true;
    }
    public function Participant_detail()
    {
        $res = $this->system2->detail_Participant($this->input->post("order_id"));
        return true;
    }
}
